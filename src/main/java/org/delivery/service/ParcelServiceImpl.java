...
public class ParcelServiceImpl implements ParcelService {
    ...
    public ParcelResponse addParcel(ParcelData parcelData) {
        if (parcelRepository.get(parcelData.userId(), parcelData.trackingNumber()) != null) {
            throw new IllegalIdException("Parcel with userId=" + parcelData.userId() +
                    ", and trackingNumber=" + parcelData.trackingNumber() + " already exists");
        }

        DeliveryData delivery = deliveryRepository.get(parcelData.userId(), parcelData.trackingNumber());
        if (delivery == null) {
            throw new IllegalIdException("Delivery with userId=" + parcelData.userId() +
                    ", and trackingNumber=" + parcelData.trackingNumber() + " does not exist");
        }
